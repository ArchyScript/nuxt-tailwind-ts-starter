<template>
  <div
    class="overflow-y-auto no-scrollbar overflow-x-hidden py-6 flex flex-col flex-grow"
  >
    <div class="py-2">
      <div class="mb-8 px-4">
      ...
      <div class="flex items-center space-x-2 py-1 px-4">
        ...
        <div class="flex-1 space-y-0.5">
          <h5 class="text-warning-50 font-medium">{{ inspectorFullname }}</h5>
          <h6 class="leading-5 text-sm text-warning-500">
            {{ inspectorUserRole }}
          </h6>
        </div>
      </div>
    ...
    <div class="border-b border-secondary-800 py-6">
      <h6 class="text-sm font-light tracking-wide text-grey-200 px-4 mb-2 py-2">
        Main menu
      </h6>
      ...
      <div class="flex flex-col space-y-3">
        <nuxt-link v-for="link in mainMenu" :key="link.route" :to="link.route">
          <div
            ...
            :class="
              tabIsActive(link.route)
                ? 'bg-warning-500 text-white'
                : 'hover:bg-grey-500 hover:text-warning-300'
            "
          >
...
<script setup lang="ts">
  ...
  const inspectorUserRole = computed(() => authUser.value?.profile?.userRole)
